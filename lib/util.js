...
function fetch_rss_entries (feed_url, max_items=100, max_days_old=1) {
    const oldest_date = moment().subtract(max_days_old, 'days');

    return _fetch_rss_page(feed_url, 1, [], max_items, oldest_date);
}
...
// Assume a wordpress feed (https://codex.wordpress.org/Pagination#Adding_the_.22paged.22_parameter_to_a_query)
function _fetch_rss_page (feed_url, page, items, max_items, oldest_date) {
    let paginated_url = feed_url;
    if (page > 1) {
        paginated_url += `?paged=${page}`;
    }

    return _fetch_rss_json(paginated_url).then(feed_json => {
        const recent_enough_items = feed_json.items.filter(article => article.pubdate >= oldest_date);
        // If we've run into articles which are too old, cease crawling
        if (recent_enough_items.length !== feed_json.items.length) {
            return items.concat(recent_enough_items);
        }

        const new_items = items.concat(feed_json.items.slice(0, max_items - items.length));
        // If we've hit the desired number of items, also cease crawling
        if (new_items.length == max_items) {
            return new_items;
        }

        // Finally, only fetch more pages if this is a wordpress feed
        const generator = feed_json.meta.generator;
        if (!generator || url.parse(generator).hostname !== 'wordpress.org') {
            console.warn(`WARNING: can't handle pagination in non-wordpress RSS feed (generator ${generator})`);
            return new_items;
        }

        return _fetch_rss_page(feed_url, page + 1, new_items, max_items, oldest_date);
    });
}
