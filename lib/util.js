...
function fetch_html(uri, encoding) {
    ...
    // Allow user to override encoding for instances where a site isn't giving
    // the proper content-type. Note that `null` is a valid option here, so we
    // only check to see if the encoding was defined before setting it.
