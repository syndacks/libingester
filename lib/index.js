...
class BlogArticle extends BaseAsset {
    ...
    set_date_published(value) {
        ...
        if (!(value instanceof Date)) {
            this._date_published = new Date(value);
        }
    }
...
class NewsArticle extends BaseAsset {
    ...
    // Takes a Date or something that a Date can be constructed out of
    set_date_published(value) {
        this._date_published = value;
        if (!(value instanceof Date))
            this._date_published = new Date(value);
    ...
    render() {
        ...
        let stylesheet = sass.renderSync({
            file: path.join(__dirname, './assets/news-stylesheet.scss'),
...
class Hatch {
    constructor(name, options) {
        ...
        this._path = 'hatch_' + name.toLowerCase() + getTimestamp();

        if (options && options.argv) {
            options = Object.assign({}, this._parse_argv(options.argv));
        }
        ...
        this._is_exporting_tgz = true;
        if (options && options.no_tgz) {
          this._is_exporting_tgz = false;
        }
