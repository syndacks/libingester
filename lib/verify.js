...
function verify_metadata(metadata) {
    ...
    } else if (object_type === "DictionaryObject") {
        assert(!!metadata['word'], "metadata missing word");
        assert(!!metadata['wordDefinition'], "metadata missing wordDefinition");
    } else {
...
function verify_image_data (data) {
    const mimetype = Magic.detectType(data);
    assert(mimetype.startsWith('image/'), "data is not an image");
}
